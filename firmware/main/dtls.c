...
// Read timeout is set to 5 seconds. This can be increased or decreased as
...
bool dtls_setup(dtls_state_t *state)
{
    int ret = 0;
    ...
    // Parse certificates
    ret = mbedtls_x509_crt_parse_file(&state->all_certs, "/spiffs/cert.crt");
    if (ret < 0)
    {
    ...
    ret = mbedtls_pk_parse_keyfile(&state->private_key, "/spiffs/key.pem", NULL, mbedtls_ctr_drbg_random, &state->ctr_drbg);
    if (ret < 0)
    {
                  ...
                  const char *port)
{
    ...
    mbedtls_net_set_nonblock(&state->fd);
 ...
 */
void dtls_close(dtls_state_t *state)
{
    int ret = 0;
...
void dtls_shutdown(dtls_state_t *state)
{
