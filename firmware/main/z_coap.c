			      ...
			      const uint8_t *value, uint16_t len)
{
	...
	if (code < cpkt->delta) {
		printf("Options should be in ascending order\n");
		return -EINVAL;
	}
...
static int zread(uint8_t *data, uint16_t offset, uint16_t *pos,
	...
	*opt_len += 1U;
	...
	if (option) {
		...
		if (len > sizeof(option->value)) {
			printf("%u is > sizeof(coap_option->value)(%zu)!",
		...
		option->len = len;
		r = zread(data, *pos, pos, max_len, len, &option->value[0]);
		if (r < 0) {
			return -EINVAL;
		}
	} else {
...
static int update_descriptive_block(struct coap_block_context *ctx,
				    int block, int size)
{
	size_t new_current = GET_BLOCK_NUM(block) << (GET_BLOCK_SIZE(block) + 4);
				 ...
				 int block, int size)
{
	...
	if (GET_BLOCK_NUM(block) > 0 && GET_BLOCK_SIZE(block) != ctx->block_size) {
		return -EINVAL;
	}
